...
- Mobile sideboard: `apps/mobile` (Expo) + `apps/relay` (Cloudflare Worker/DO) + `packages/protocol` + `src-tauri/src/mobile.rs`. Root `.env` is shared: pairing uses `EXPO_PUBLIC_MOBILE_RELAY_URL`; desktop and Worker both use `MOBILE_RELAY_CREATE_SECRET`.
