...
1. **`data-hit` 属性によるヒット領域。** OS のクリックスルーはウィンドウ単位でしか切れないため、フロントが可視の `[data-hit]` 要素の rect を `invoke("set_hit_regions")` で報告し、バックエンドがカーソル位置と照合してクリックスルーを切り替えている。`data-hit` を付ける要素は現状と同一にする:
   ...
   - OPENLOL パネルのルート (`#openlol` 相当、パネル全体)
...
3. **`pointer-events` の構造。** `html, body { pointer-events: none }` が基本で、`.recs` / `.settings` / `.openlol` のパネルルートだけ `pointer-events: auto`。これを崩すとゲームへのクリックが吸われる。
4. **パネルドラッグの実装。** in-game パネルは pointer capture による自前ドラッグ(`style.left/top` 直接書き換え)+ ドラッグ中は `invoke("set_drag_active", { active: true })` でウィンドウを操作可能に固定し、終了時に `false` + 位置保存(`set_ingame_panel_position`) + `reportHitRegions()`。OPENLOL ヘッダーは `appWindow.startDragging()` による**ウィンドウごと**のドラッグで、`onMoved` を 250ms デバウンスして `set_champselect_window_position` に保存(champselect モード時のみ)。`shouldStartDrag` のインタラクティブ要素除外(`button, input, label, select, textarea, a`)も維持。
...
- `champselect.ts`: OPENLOL パネル。モジュールレベル `let` の状態 + `scheduleRender()`(microtask で renderAll を coalesce) + セクションごとの render-key guard(入力が変わったときだけ DOM 再構築)という**自前リアクティブシステム**。これが Solid のシグナルでそのまま置き換わる。
...
src/
  ...
  components/
    ...
    openlol/
      OpenLolPanel.tsx  表示判定 + ヘッダー (ウィンドウドラッグ, phase チップ, pin, gear)
...
3. パネル単位で変換する。順番: StatusChip → LpBanner/RuneBanner → SettingsPanel → InGamePanel → OPENLOL 一式。1パネル変換するごとに styles.css から対応ブロックを削除し、モックモードで見比べる。
...
### champselect.ts → state/ + components/openlol/
...
- [ ] OPENLOL: champ-select 突入でパネル表示、phase チップ更新
- [ ] OPENLOL: STRONG PICKS / BAN TARGETS のスケルトン → データ表示、WR/Δ/games 列
- [ ] OPENLOL: 行ホバーでビルドエリアがプレビューに切り替わり、ホバー解除で自分のピックに戻る
- [ ] OPENLOL: ロールチップ(ロール未確定時のみ表示)切替でリスト更新
- [ ] OPENLOL: 敵スロット表示、カウンターストリップ、カウンターホバーのプレビュー
- [ ] OPENLOL: BEST/VS タブ切替、VS ドロップダウンで対象変更、外側クリックで閉じる
- [ ] OPENLOL: ルーンページ(ツリーヘッダー色、キーストーン、シャード)、stats 行(WR/games/スペル)、FLIP でスペル順反転
- [ ] OPENLOL: IMPORT ボタンの idle→importing→imported/failed 遷移と文言
- [ ] OPENLOL: pin トグルで champ-select 終了後もパネル維持
- [ ] OPENLOL: ヘッダードラッグでウィンドウ移動
