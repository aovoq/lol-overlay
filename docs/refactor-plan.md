...
- u.gg の VS matchup rune は stats2 の `overview/.../matchups/{champion_key}_{enemy_key}` から取得する。薄い matchup は `NotEnoughData` を返す。
...
Fallback / provider selection policy: `docs/data-provider-policy.md` を正とする。
...
マッチアップ別ビルド概要:
  https://stats2.u.gg/lol/1.5/{build}/{patch}/{mode}/matchups/{champion_key}_{enemy_key}/{api_version}.json
  例: https://stats2.u.gg/lol/1.5/overview/16_12/ranked_solo_5x5/matchups/8_103/1.5.0.json

サイト全体 tier list:
  https://stats2.u.gg/lol/1.5/champion_ranking/{region}/{patch}/{mode}/{rank_tier}/{api_version}.json
  例: https://stats2.u.gg/lol/1.5/champion_ranking/world/16_12/ranked_solo_5x5/emerald_plus/1.5.0.json
```
...
- `platform_id: RwLock<String>`(default は `"KR"`。`set_platform_id` で更新)
...
統計 region の default は KR。`set_platform_id` が呼ばれた場合はその platform を使う。overview / matchups は選択 region → 無ければ World、tier list は `champion_ranking/{region}/...` を使う。ランクは `Rank::preferred_order()` の先頭ヒット。ロールは指定ロール → 無ければデータ最多ロール。

### 11.3 `BuildProvider` メソッド対応表

| trait メソッド | u.gg データ | 実装メモ |
|---|---|---|
| `set_platform_id` | — | default は KR。LCU から明示 platform が来たらその region に更新 |
...
| `tier_list(role)` | `champion_ranking` の role rows | Emerald+ 固定。role 内 pick/ban weight から rate を正規化し、前 patch の `champion_ranking` が取れた場合だけ `win_rate_delta` を percentage points で埋める。低 pick row は除外 |
...
| `rune_build(champ, role, enemy)` | 通常は overview、敵指定時は `overview/.../matchups/{champion_key}_{enemy_key}` の `runes` + `shards.shard_ids` + `summoner_spells` + wins/matches | 敵指定時は `matchup: true`。薄い matchup や未対応 shape は `Err(NotEnoughData)` / provider error |
| `champion_names(id)` | — | `overlay-ddragon` の `id_to_name` / `id_to_image` から(deeplol と同一ソースなので表示も同一) |

### 11.4 テスト

- 単体: `Q/W/E/R` 変換、platform_id → Region マップ、provider の default region が KR であること、patch 文字列変換(`"15.12.1"` → `"15_12"`)、counters の視点反転と 30 games フィルタ、tier list role 変換 / delta / 低 pick row フィルタ、visitor 移植分のフィクスチャパース(参考実装のテストがあれば流用)。
