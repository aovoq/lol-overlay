...
//! and the per-lane tier list (see [`crate::flight`] module docs for why only
//! some sections have one).
//!
...
//! own `pick_rate`/`win_rate` are 0..1 fractions, while every percentage
//! nested inside it ([`Perk::win_rate`] and friends, via
//! `#[serde(default)]`-less plain `f64` fields not modeled here),
//! [`CounterRow::win_rate`], and every [`TierRow`] rate are 0..100. Each field
//! doc below says which.
